...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	...
	GLint loc_quad = glGetUniformLocation(shader->program, "is_quad");
	glUniform1i(loc_quad, isQuad);

	glUniform1i(glGetUniformLocation(shader->program, "textura_mixta"), mixTexts);
