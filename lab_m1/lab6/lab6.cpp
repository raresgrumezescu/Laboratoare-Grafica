...
Mesh* Lab6::CreateMesh(const char *name, const std::vector<VertexFormat> &vertices, const std::vector<unsigned int> &indices)
{
	...
	/*Cum functiile glEnableVertexAttribArray si  glVertexAttribPointer primesc ca parametru pipe-ul,
	(prima functie il seteaza, iar a doua il primeste ca prim parametru), tot ce am facut a fost sa modific cifra 3 cu 1
	si viceversa, ca sa schimb pipe-urile*/

    // Set vertex normal attribute
    glEnableVertexAttribArray(3);
	glm::vec3 v_positions;
	v_positions.x = 0;
	v_positions.y = 1;
	v_positions.z = 1;
	//double a = 0.4
	...
	//glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(VertexFormat), &v_positions);
    // Set texture coordinate attribute
    glEnableVertexAttribArray(2);
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(VertexFormat), (void*)(2 * sizeof(glm::vec3)));
	//glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(VertexFormat), (void*)0);
...
void Lab6::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix)
{
	...
	/*Am folosit funtia glUniformMatrix4fv, dupa ce am obtinut locatia variabilei cu functia glGetUniformLocation.
	locatia o trimit ca prim parametru primei functii amintite. Functiile le apelez conform
	specificatiilor din enuntul laboratorului.*/

    // TODO(student): Get shader location for uniform mat4 "Model"
	GLint mL = glGetUniformLocation(shader->GetProgramID(), "Model");
    // TODO(student): Set shader uniform "Model" to modelMatrix
	//glUniformMatrix4fv(mL, 1, GL_TRUE, glm::value_ptr(modelMatrix));
	glUniformMatrix4fv(mL, 1, GL_FALSE, glm::value_ptr(modelMatrix));
    // TODO(student): Get shader location for uniform mat4 "View"
	GLint vL = glGetUniformLocation(shader->GetProgramID(), "View");
    // TODO(student): Set shader uniform "View" to viewMatrix
    glm::mat4 viewMatrix = GetSceneCamera()->GetViewMatrix();
	glUniformMatrix4fv(vL, 1, GL_FALSE, glm::value_ptr(viewMatrix));
	//glUniformMatrix4fv(vL, 1, GL_TRUE, glm::value_ptr(viewMatrix));
	// TODO(student): Get shader location for uniform mat4 "Projection"
	GLint pL = glGetUniformLocation(shader->GetProgramID(), "Projection");
    // TODO(student): Set shader uniform "Projection" to projectionMatrix
    glm::mat4 projectionMatrix = GetSceneCamera()->GetProjectionMatrix();
	glUniformMatrix4fv(pL, 1, GL_FALSE, glm::value_ptr(projectionMatrix));
	//glUniformMatrix4fv(pL, 1, GL_TRUE, glm::value_ptr(projectionMatrix));
